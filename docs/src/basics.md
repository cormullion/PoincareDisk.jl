...
In Julia you can use the `cis()` function to generate complex numbers $e^im x$. It calculates $\exp(i x)$ using Euler's formula, $\cos(x) + i \sin(x)$.
...
The size of the disk (in terms of the Luxor drawing) is determined by the global constant `DEFAULT_DISK_RADIUS`, which is initially set to 295.0 (so that the Poincare disk fits neatly on the default Luxor drawing size of 600 × 600). The default action is `:stroke`, or you can draw a filled disk with `action=:fill`.
...
The default action is `:stroke` - `:fill` is not useful here, but `:path` would allow you to obtain the path for later use.
