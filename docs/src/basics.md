...
The Poincaré disk is a two-dimensional hyperbolic space. It provides a surface on which you can draw using *non-Euclidean geometry*, where most of Euclid's geometric postulates apply, but the final (fifth) postulate doesn't. You'll have no trouble finding explanatory material on the internet!
...
All graphics functions are provided by Luxor.jl, and you can find the documentation for that package [here](https://juliagraphics.github.io/LuxorManual/stable/).

The Luxor.jl method of supplying an *action* to a function, such as `:stroke` or `:fill` is used here too.
...
This example draws points on the disk using the form `center + radius * θ`. 
...
You can add graphics for the Poincaré unit disk itself with:

[`draw_poincare_disk(action=:fill)`](@ref)

The size of the unit disk (in terms of the Luxor drawing) is determined by the global constant `DEFAULT_DISK_RADIUS`, which is initially set to 295.0 (so that the Poincare disk fits neatly on a default Luxor drawing). The default action is `:stroke`.

## Hyperbolic lines

Use `hyperbolic_line(z1, z2)` to construct a hyperbolic line between `z1` and `z2`.

These lines are called *geodesics*: they're arcs of circles that meet the unit circle orthogonally.
...
This function, like its Luxor Euclidean counterpart, expects an array of complex number coordinates. 
...
A number of the functions have the `radius=` keyword:
...
The utility function [`regular_hyperbolic_polygon_vertices()`](@ref) generates a regular hyperbolic polygon, inscribed in the hyperbolic circle of the given radius.
...
sethue("white")
for i in 0.1:0.1:6
    vs = regular_hyperbolic_polygon_vertices(5, i, rotation = -π/2)
